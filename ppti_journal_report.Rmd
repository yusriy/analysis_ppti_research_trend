---
title: "Strategic Journal Targeting"
subtitle: "Evidence-Based Journal Selection from Recent Publications"
institute: "School of Industrial Technology, Universiti Sains Malaysia"
pagetitle: "Strategic Journal Targeting | PPTI"
author: 
...
.stat-card {
  border-top: 6px solid #111827;  /* charcoal */
  background: #ffffff;
...
.stat-card::before{
  background: #111827 !important;  /* or blue/green/etc */
}
...
This report presents an evidence-based overview of journals in which PPTI academics
have successfully published in the last two years. It is intended to support:

- **Strategic journal targeting** for new manuscript submissions  
- **Reduction of desk rejections** through scope and outlet alignment  
- **Informed publication planning** aligned with PPTI research strengths  

This document should be used as a **living reference**, updated periodically to reflect
emerging publication trends and shifts in journal performance.

### Report Objective

This report lists journals where PPTI lecturers published within the **last two years**. It serves as a **recommended journal targeting reference** for future manuscript submissions.
...
```{r}
pub_data <- read_csv("data/ppti_journal_publications.csv")
summary_data <- read_csv("data/journal_publication_summary.csv")
...
start_date <- end_date %m-% months(24) + days(1)
...
```{r}
# Extract Unique Journals Only (for summary counts)
...
```{r, echo=FALSE}
#cutoff_text <- "2023–2025"

#cutoff_text <- paste0(year(start_date), "–", year(end_date))
      ...
      tags$div(class = "card-body",
        tags$div(class = "d-flex justify-content-between align-items-start",
          ...
          tags$span(class = "badge badge-pill",
                    style = "background:rgba(99,102,241,.15); color:#3730a3;")
        )
      )
    )
  ),
    ...
    tags$div(class = "card stat-card",
      tags$div(class = "card-body",
        tags$div(class = "d-flex justify-content-between align-items-start",
          ...
          tags$span(class = "badge badge-pill",
                    style = "background:rgba(34,197,94,.15); color:#166534;")
        )
      )
    )
  ),
    ...
    tags$div(class = "card stat-card",
      tags$div(class = "card-body",
        tags$div(class = "d-flex justify-content-between align-items-start",
          ...
          tags$span(class = "badge badge-pill",
                    style = "background:rgba(236,72,153,.12); color:#9d174d;")
        )
      )
    )
  )
...
```
# Recommended Journal Target List

---

## Interpretation Guide
The terms used in this report is defined below.
...
ggplot(top10, aes(x = Publication_Count, y = reorder(Journal, Publication_Count), fill = Publication_Count)) +
  ...
  labs(
    title = "Top 10 Journals by Publication",
